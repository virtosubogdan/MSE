...
let rec has a map=SM.exists (fun k v->a=k) map;;
let pTrue=Fun("true",[]);;
...
and incearca c kB rest pred map=function
 (rule,expand)::t->
   (match rename c [rule] with [renamedRule] -> 
     ...
     with Failure(esec)-> if d then Printf.printf "incearca fail:%s\n" esec)
     |[]->failwith "imposibru");
...
and eval c kb list map=match list with 
    h::t-> (try (if h=pTrue 
      ...
      else incearca c kb t h map kb)  with Failure(reason) -> if d then Printf.printf "%s\n" reason) 
  |[]->printmap map;;


eval 0 [ (Fun("parinte",[Fun("tataion",[]) ;Fun("ion",[])] ),[pTrue]);
        (Fun("parinte",[Fun("alina",[]) ;Fun("andrei",[])] ),[pTrue]);
	(Fun("parinte",[Fun("ion",[]) ;Fun("andrei",[])] ),[pTrue]);
	(Fun("parinte",[Fun("andrei",[]);Fun("fiuAndrei",[])]),[pTrue]);
	(Fun("descendent",[Var("x");Var("y")]),
	 [Fun("parinte",[Var("x");Var("y")])]);
	(Fun("descendent",[Var("x");Var("y")]),
	 [Fun("parinte",[Var("x");Var("z")]);Fun("descendent",[Var("z");Var("y")])])
     ]
  [Fun("descendent",[Var("x");Fun("fiuAndreii",[])])]
 SM.empty;;
